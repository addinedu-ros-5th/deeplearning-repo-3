...
class TrafficLightDetector:
    ...
    def check_driving_status(self): #주행 == true 주행  == false
        ...
        if len(self.size_history) == self.history_length:
            ...
            print(f"recent_changes: {recent_changes}")
...
if __name__ == "__main__":
    ...
    car_stop_threshold = 70000    # 자동차 정지 크기 임계값
    increase_rate_threshold = 25000 # 자동차 크기 증가율 임계값
    ...
    car_size_history = deque(maxlen=30)
    ...
    while cap.isOpened():
        ...
        # 자동차 크기 변화율 확인
        car_size_history.extend(car_sizes)
        if len(car_size_history) == car_size_history.maxlen:
            recent_car_sizes = list(car_size_history)
            increase_rate = max(recent_car_sizes) - min(recent_car_sizes)
            print('increase_rate:', increase_rate)
            if (max(recent_car_sizes) > car_stop_threshold or increase_rate < increase_rate_threshold):
                print('recent_car_sizes:', recent_car_sizes)
                is_driving = False
            else:
                is_driving = True
        else:
        ...
        if traffic_detector.signal_color is not None:
            ...
            cv2.putText(frame, f"Signal color: None", (20, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2, cv2.LINE_AA)
            cv2.putText(frame, f"Pass/Fail: None", (20, 150), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2, cv2.LINE_AA)
